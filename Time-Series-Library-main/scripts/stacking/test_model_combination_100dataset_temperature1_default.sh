...
model_combinations=(
    ...
    "DLinear|||{\"e_layers\": 1, \"d_layers\": 1}"
    "iTransformer|||{\"e_layers\": 2, \"d_layers\": 1, \"d_model\": 128, \"d_ff\": 128}"
    "FreTS|||{\"e_layers\": 1, \"d_layers\": 1, \"n_heads\": 2}"
    "Autoformer|||{\"e_layers\": 2, \"d_layers\": 1}"
    "TimesNet|||{\"e_layers\": 2, \"d_layers\": 1, \"d_model\": 16, \"d_ff\": 32, \"top_k\": 5}"
    "SegRNN|||{\"seg_len\": 28, \"d_model\": 512, \"dropout\": 0.5, \"learning_rate\": 0.0001}"
...
for combination in "${model_combinations[@]}"; do
    ...
    for repo in "${repoName[@]}"; do
        ...
        --result_path workresult/stacking/generalization_test/attention/exp_"$meta_model"_${model_name}_100dataset \
        --use_context_aware \
        --d_model 512\
        --n_heads 8 \
