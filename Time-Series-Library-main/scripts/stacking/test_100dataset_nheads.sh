...
for n_heads in "${n_heads_values[@]}"; do
    echo "Testing with n_heads: $n_heads"
    ...
    for repo in "${repoName[@]}"; do
        ...
        --result_path workresult/stacking/Parameter_Sensitivity_Analysis/exp_"$meta_model"_noHistoricalEncoder_noResidual_2model_PatchTST_Dlinear_100dataset_nheads_"$n_heads" \
        --use_context_aware \
        ...
        --des 'Exp' \
        --inverse \
        --itr 1 | tee -a logs/stacking/stacking_test_100_nheads_"$n_heads".log
    done
done